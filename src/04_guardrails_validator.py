...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        # Lặp qua các loại PII và pattern tương ứng
        for pii_type, pattern in self.PII_PATTERNS.items():
            # Tìm tất cả matches
            matches = re.findall(pattern, value)

            for match in matches:
                # Thay thế match bằng placeholder "[PII_TYPE_REDACTED]"
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            # Không thể sửa → trả về JSON lỗi dự phòng (fallback error JSON)
            fallback = json.dumps(
...
def demo_pii_guard():
    ...
    # Tạo Guard với PIIDetector — on_fail truyền vào CONSTRUCTOR của validator
...
def demo_json_guard():
    ...
    # Tạo Guard với JSONFormatter — on_fail truyền vào CONSTRUCTOR của validator
