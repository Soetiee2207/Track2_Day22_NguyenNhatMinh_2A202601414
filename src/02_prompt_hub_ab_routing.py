...
# Đặt tên duy nhất theo tên sinh viên để tránh trùng
...
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện. Trả lời ngắn gọn, rõ ràng (2-4 câu) "
    "dựa trên context sau. Nếu không có thông tin trong context, "
...
# V2: phong cách chuyên nghiệp, có cấu trúc (3-5 câu)
SYSTEM_V2 = (
    "Bạn là chuyên gia phân tích thông tin AI. Khi trả lời, hãy thực hiện: "
    "1) Xác định các facts liên quan trong context; "
    "2) Viết câu trả lời có tổ chức, mạch lạc 3-5 câu; "
    "3) Chỉ dùng thông tin từ context, không suy đoán thêm. "
    "Nếu context không đủ dữ liệu, nêu rõ điều đó.\n\n"
    "Context:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    """
    # Push PROMPT_V1 lên Hub
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME, object=PROMPT_V1,
    ...
    # Push PROMPT_V2 lên Hub
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME, object=PROMPT_V2,
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    # Pull PROMPT_V1_NAME từ Hub, fallback về local nếu lỗi
    try:
    ...
    # Pull PROMPT_V2_NAME từ Hub, fallback về local nếu lỗi
    try:
...
def get_prompt_version(request_id: str) -> str:
    ...
    """
    # Tính MD5 hash của request_id → số nguyên (deterministic)
    hash_int = int(hashlib.md5(request_id.encode()).hexdigest(), 16)
...
def main():
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        # Định tuyến tất định theo hash của request_id
        version_key = get_prompt_version(request_id)
        version_tag = "v1" if version_key == PROMPT_V1_NAME else "v2"
        prompt      = prompts[version_key]

        # Chạy query có trace với prompt version đã chọn
