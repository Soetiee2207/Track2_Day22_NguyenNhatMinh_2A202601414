...
# Phải GIỐNG HỆT SYSTEM_V1/SYSTEM_V2 trong 02_prompt_hub_ab_routing.py
SYSTEM_V1 = (
...
def run_rag(retriever, llm, prompt, question: str) -> dict:
    ...
    # contexts là danh sách page_content (KHÔNG ghép chuỗi — RAGAS cần list[str])
    ...
    # Trả về dict với answer và contexts (list[str])
...
def collect_rag_outputs(vectorstore, prompt_version: str) -> list:
    ...
    for i, qa in enumerate(QA_PAIRS, 1):
        # Chạy RAG cho câu hỏi hiện tại
        out = run_rag(retriever, llm, prompt, qa["question"])

        # Lưu kết quả với 4 keys
        results.append({
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    # Chạy evaluate() với đầy đủ 4 metrics
    result = evaluate(
        dataset,
